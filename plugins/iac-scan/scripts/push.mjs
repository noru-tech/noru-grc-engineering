#!/usr/bin/env node
// :push for the iac-scan piece — contract requirements 4 and 5.
//
...
//   4. emit the exact, ordered tool calls to .noru/.cache/iac-scan.calls.json
//
...
function main(argv) {
  ...
  if (!opts.confirm) {
    process.stderr.write(
      "error: refusing to push without --confirm.\n" +
        `  ${plan.summary.create} finding(s) would be filed and ${plan.summary.update} updated or ` +
          "closed in Noru.\n" +
  ...
  if (opts.json) {
    ...
    if (calls.length === 0) {
      process.stdout.write(
        "nothing to push: every finding in the plan is already filed in Noru with exactly this\n" +
          "content, and nothing is waiting to be closed.\n" +
