#!/usr/bin/env node
// :diff for the iac-scan piece — contract requirement 5. Reads only; writes nothing to Noru.
//
// Inputs, both local:
//   .noru/.cache/iac-scan.parsed.json  the validated manifest (from validate_manifest.py)
...
const MARKER_PREFIX = "noru-grc-engineering:iac-scan";
 ...
 * TODO: replace this with the real create/update/skip decision for your piece, and make sure every
 * operation declares the idempotency that makes a second run a no-op.
 */
export function buildOperations(manifest, state) {
  const evidence = state.evidence ?? [];
  const src = manifest.source;
  return (manifest.items ?? []).map((item) => {
    const marker = `[${MARKER_PREFIX}#${item.key}]`;
    ...
    return {
      ...
      reason: existing
        ? `evidence ${existing.id} already carries this marker`
        : "no evidence carries this marker yet",
      idempotency: { kind: "client_probe", key: ["description contains marker"], marker },
      arguments: {
        title: item.title,
        description: `${marker} ${item.kind} from ${src.slug} @ ${src.commit_sha} (${src.branch}).`,
        content: [
          `${item.kind}: ${item.title}`,
          `Owner: ${item.interpretation.owner}`,
          `Decided: ${item.interpretation.decided_at}`,
          `Expires: ${item.interpretation.expires_at ?? "(not set)"}`,
          `Rationale: ${item.interpretation.rationale}`,
          "",
          "Repository evidence:",
          ...(item.refs ?? []).map((r) => `  - ${r}`),
        ].join("\n"),
...
function main(argv) {
  ...
  const parsed = loadJson(
    join(opts.repo, ".noru", ".cache", "iac-scan.parsed.json"),
    "validated manifest"
  );
  if (parsed.error) {
    process.stderr.write(
