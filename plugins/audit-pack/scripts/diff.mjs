#!/usr/bin/env node
// :diff for the audit-pack piece — contract requirement 5. Reads only; writes nothing to Noru.
//
// Inputs, both local:
//   .noru/.cache/audit-pack.parsed.json  the validated manifest (from validate_manifest.py)
...
function main(argv) {
  ...
  const parsed = loadJson(
    join(opts.repo, ".noru", ".cache", "audit-pack.parsed.json"),
    "validated manifest"
  );
  if (parsed.error) {
    process.stderr.write(
