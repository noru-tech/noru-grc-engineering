...
1. `/audit-pack:diff` must have been run and its plan reviewed.
...
Then execute exactly the calls in `.noru/.cache/audit-pack.calls.json`, in order, and nothing
else. Do not improvise a call, do not reorder, do not retry a write on a 5xx without telling
the user.

Afterwards re-run `/audit-pack:diff`: every operation should be `skip`.
