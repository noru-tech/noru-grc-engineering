#!/usr/bin/env node
// :push for the governance-records piece — contract requirements 4 and 5.
//
...
//   4. emit the exact, ordered tool calls to .noru/.cache/governance-records.calls.json
//
...
function main(argv) {
  ...
  if (opts.json) {
    ...
    if (calls.length === 0) {
      process.stdout.write(
        "nothing to push: every record in the plan is already filed in Noru.\n" +
